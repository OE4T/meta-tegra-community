...
do_install() {
    ...
    ln -sf libnvds_msgconv_mega.so.1.0.0 ${D}${DEEPSTREAM_PATH}/lib/libnvds_msgconv_mega.so
    ...
    protobuf_lite_soname=$(${OBJDUMP} -p ${STAGING_LIBDIR}/libprotobuf-lite.so | grep SONAME | awk '{print $2}')
    ...
    patchelf --replace-needed libnvvpi.so.3 libnvvpi.so.4 ${D}${DEEPSTREAM_PATH}/lib/libnvds_opticalflow_orin.so
    bbnote "Patching libgrpc++ NEEDED to $grpcpp_soname, libgrpc to $grpc_soname"
    ...
    bbnote "Patching libprotobuf NEEDED to $protobuf_soname, libprotobuf-lite to $protobuf_lite_soname"
    ...
    patchelf --replace-needed libprotobuf-lite.so.3.21.12.0 $protobuf_lite_soname ${D}${DEEPSTREAM_PATH}/lib/libnvds_riva_asr_grpc.so
    patchelf --replace-needed libprotobuf-lite.so.3.21.12.0 $protobuf_lite_soname ${D}${DEEPSTREAM_PATH}/lib/libnvds_riva_tts.so
    patchelf --replace-needed libgpr.so.31 libgpr.so.53 ${D}${DEEPSTREAM_PATH}/lib/libnvds_riva_tts.so
    ...
    patchelf --replace-needed libgpr.so.31 libgpr.so.53 ${D}${DEEPSTREAM_PATH}/lib/libnvds_riva_asr_grpc.so
...
FILES:${PN}-nmos = "${DEEPSTREAM_PATH}/lib/libnvds_nmos*"
FILES:${PN}-triton = "\
