...
cleaning() {
    ...
    # Cleaning in mlp folder
    for i in {0..9}; do
        if [ -e "${SAMPLEDATA}/mlp/$i.pgm" ]; then
            rm -f "${SAMPLEDATA}/mlp/$i.pgm" 
        fi
    done
}
...
run_mlp() {
    echo "Copying ${SAMPLEDATA}/mnist/*.pgm to ${SAMPLEDATA}/mlp"
    cp ${SAMPLEDATA}/mnist/*.pgm ${SAMPLEDATA}/mlp/
